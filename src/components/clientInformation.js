...
export default function ClientInformation() {
  ...
  const createPackage = async e => {
    ...
    //console.log("olha o prod", product_id)
  ...
  async function getMotvCustomer() {
    ...
    //console.log("olha o vendorname", vendorName)
      let resultado = await fetch("http://10.5.0.53/api/v1/motvCustomer?id="+user_id, {
          method: 'GET',
          headers: {
              Authorization: tokenH,
          },
  ...
  async function getAllowedProductCustomer() {
    ...
    //console.log("olha o product customer", vendorName)
      let resultado = await fetch("http://10.5.0.53/api/v1/motvSubscription/allowed/products?id="+user_id, {
          method: 'GET',
          headers: {
              Authorization: tokenH,
          },
  ...
  async function getSubscriptionInfoCustomer() {
    setLoading(true);
    //console.log("olha o product customer", vendorName)
      let resultado = await fetch("http://10.5.0.53/api/v1/motvSubscription?id="+user_id, {
          method: 'GET',
          headers: {
              Authorization: tokenH,
          },
          ...
          <Grid container spacing={3}>
              ...
              .map((item) => (
                ...
                <Paper className={classes.paper2}>
                  ...
                  <button style={{width: '40px', height: '30px', }} type="submit" onClick={ (e) => setProductId2(item.productId)} className="click">
              ...
              {allowedProduct.map((item) => (
                ...
                <Paper className={classes.paper2}>
                  ...
                  <div style={{display: 'flex', width: '80%', height: '50px', margin: 'auto auto auto 20%', justifyContent: 'space-between', alignItems: 'center'}}> 
                    ...
                    <Tooltip title="Adicionar pacote ao cliente" placement="right">
                  <button style={{width: '40px', height: '30px', }} type="submit" onClick={ (e) => setProduct(item.productId)} className="click">
                    <AddBoxIcon style={{
                          color: '#FBCF00',
                          margin: 'auto',
                          width: 30,
